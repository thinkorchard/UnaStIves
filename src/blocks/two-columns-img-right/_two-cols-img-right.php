<?php
	...
	function two_cols_img_right_block_register_block() {

		acf_register_block_type( array(
			'name'            => 'two-cols-img-right',
			'title'           => __( 'Two Columns Image Right Block' ),
			'description'     => __( 'Display a block with both with 2 columns with Image on right' ),
			'render_template' => 'src/blocks/two-columns-img-right/two-cols-img-right.php',
			'category'        => 'una-blocks',
...
// Check if function exists and hook into setup and adds all blocks.
	if ( function_exists( 'acf_register_block_type' ) ) {
		add_action( 'acf/init', 'two_cols_img_right_block_register_block' );
	}
