...
if ( ! function_exists( 'ign_posted_by' ) ) {
	function ign_posted_by( $post_id, $avatar_size = 50 ) {
		...
		$author_name        = sprintf( __( '%s by %s', 'una' ), '<a href="' . $author_link . '" class="author-name"><span class="byline">', '</span>' . $author_meta( 'author_name' ) . '</a>' );
...
if ( ! function_exists( 'ign_time_link' ) ) {
	...
	function ign_time_link( $post_id = '', $date_format ) {
		...
		/* translators: %s: post date */
			__( '<span class="screen-reader-text">Posted on</span> %s', 'una' ),
	...
	function ign_edit_link( $id = null, $class = '', $text = 'Edit' ) {
		...
		edit_post_link(
			...
			/* translators: %s: Name of current post */
				__( '%s<span class="screen-reader-text"> "%s"</span>', 'una' ),
				$text, get_the_title()
			),
